...
export default function TextFieldAnalysis() {
    ...
    const analyzeSentiment = async () => {
        ...
        // ตัวอย่าง logic mock API หลอก ๆ  (คุณค่อยไปเชื่อม backend จริง)
        setTimeout(() => {
        const lower = text.toLowerCase();

        if (lower.includes("good") || lower.includes("love")) {
            setResult("Positive");
        } else if (lower.includes("bad") || lower.includes("hate")) {
            setResult("Negative");
        } else {
            setResult("Neutral");
        }
                    ...
                    {result && text && (
                        <div className="bg-gray-100 rounded-lg text-black p-4">
                            <div className="flex items-center">
                                <p>Sentiment</p>
                                <div className={`flex items-center w-24 h-6 justify-center text-sm ml-auto rounded-full text-white ${mappedCard?.color ?? "bg-gray-400"}`}>{mappedCard?.result ?? "Unknow"}</div>
                            </div>
                        </div>
                    )}
