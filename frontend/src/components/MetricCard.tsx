        ...
        setMetrics([
          {
            id: 1,
            name: "Accuracy",
            value: data.accuracy ? fmt(data.accuracy) : "-",
          },
          {
            id: 2,
            name: "Precision",
            value: data.macro_precision ? fmt(data.macro_precision) : "-",
          },
          {
            id: 3,
            name: "Recall",
            value: data.macro_recall ? fmt(data.macro_recall) : "-",
          },
          {
            id: 4,
            name: "F1 Score",
            value: data.micro_f1 ? fmt(data.micro_f1) : "-",
          },
        ]);
      } catch (err) {
    ...
    <div className="col-start-3 col-end-11 row-span-3 space-y-4 self-center">
      <div className="flex space-x-4  ">
        {metrics.map((metric) => (
          <div
            key={metric.id}
            className="flex flex-col w-1/4 justify-center  bg-white rounded-xl space-y-2 p-4"
          >
            <p>{metric.name}</p>
